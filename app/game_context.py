...
totals, pace-of-play baseline) to each Game in a schedule, and -- once a
game is final -- how far the actual result landed above or below each of
those lines. All of it comes from nflverse (app.nflverse_client); nothing
...
from app.models import Game, GameContext, PaceStat, WeekSchedule
...
def _pace_stat(actual: float | None, baseline: float | None) -> PaceStat | None:
    if actual is None and baseline is None:
        return None
    delta = round(actual - baseline, 1) if actual is not None and baseline is not None else None
    return PaceStat(actual_plays=actual, baseline_plays=round(baseline, 1) if baseline is not None else None, delta=delta)


async def _build_context(game: Game, line_row: dict | None) -> GameContext | None:
    if line_row is None:
        return None
    ...
    if is_final:
        ...
        away_pace = _pace_stat(away_actual, away_baseline)
        home_pace = _pace_stat(home_actual, home_baseline)
...
async def attach_game_context(schedule: WeekSchedule) -> None:
    ...
    async def enrich(game: Game) -> None:
        ...
        try:
            game.context = await _build_context(game, line_row)
        except Exception:
            game.context = None
